...
class FieldTemplateSelector:
    def __init__(self):
        ...
        self.template_bank_path = "/Users/jaiharishsatheshkumar/synthetic_data_generator/template_bank.json"
        self.template_bank = self._load_template_bank(self.template_bank_path)

        data_path = "/Users/jaiharishsatheshkumar/synthetic_data_generator/.venv/bin/enriched_employee_dataset_50000.json"
        logs_path = "logs.json"
    ...
    def generate_small_logs(self, input_json_path, output_log_path, desired_log_count):
        try:
            ...
            for user_record in records[:desired_log_count]:
                user_id = str(user_record.get('Employee ID', 'unknown'))
                fields = [f for f in user_record if f != 'Employee ID' and user_record[f]]
                if not fields:
                    continue
    ...
    def select_top_3_questions_for_random_field(self, user_id):
        """
        Select a random data field from the template bank,
        then select top 3 question templates for that field,
        avoiding repetition using RLSelector.
        """
        ...
        # Randomly pick a field from the template bank keys
        selected_field = random.choice(list(self.template_bank.keys()))
        templates_for_field = self.template_bank[selected_field]

        # Build candidates: list of (field, template) for all templates of the field
        candidates = [(selected_field, t) for t in templates_for_field]

        # Use RL selector to pick top 3 templates avoiding recent repeats
        top_templates = self.rl_selector.select_best(user_id, candidates, k=3)

        # Return the field and top 3 question templates (strings)
        top_questions = [t for _, t in top_templates]

        return selected_field, top_questions
